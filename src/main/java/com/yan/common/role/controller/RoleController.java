...
import com.yan.common.role.model.*;
...
public class RoleController extends BaseController {
	...
	@MapperInject(UserRoleRelMapper.class)
	private UserRoleRelMapper mapperUserRoleRel;
	...
	public List<RoleNode> getRoleTree(String id) {
		...
		UserRoleRelExample.Criteria criteriaUserRoleRel = userRoleRelExample.createCriteria();
		criteriaUserRoleRel.andUserIdEqualTo(userID);
		UserRoleRel userRoleRel = mapperUserRoleRel.selectByExample(userRoleRelExample).get(0);
		String roleID = userRoleRel.getRoleId();

		SysRoleExample sysRoleExample = new SysRoleExample();
		SysRoleExample.Criteria criteriaSysRole = sysRoleExample.createCriteria();
		criteriaSysRole.andRoleIdEqualTo(roleID);
		SysRole sysRole = mapper.selectByExample(sysRoleExample).get(0);

		if (this.isNull(sysRole)) {
			id = "00000000000000000000000000000001";
		}else {
			id = sysRole.getRolePid();
		}

		List<RoleNode> nodeList = new ArrayList<>();
		List<RoleNode> pNodeList = new ArrayList<>();
		RoleNode PRoleNode =  delegateMapper.selectOne(NAMESPACE + ".getOneRoleNode", id);
		...
		PRoleNode.setChildren(nodeList);
		pNodeList.add(PRoleNode);
		return pNodeList;
	}

	/**
	 * 获取 checkbox 形式的 tree 结构（为用户管理提供支持，用户只能管理其同目录或子目录的用户）<br>
	 *
	 * @param userId 用户Id
	 * @param pid 父角色Id
	 * @return List<RoleNode> 角色节点列表集合
	...
	@ResponseBody
	public List<RoleNode> getRoleCheckedTree(String userId, String pid) {
		...
		criteriaSysRole.andRoleIdEqualTo(roleID);
		SysRole sysRole = mapper.selectByExample(sysRoleExample).get(0);
		String rolePID = sysRole.getRolePid();
		pid = rolePID;
